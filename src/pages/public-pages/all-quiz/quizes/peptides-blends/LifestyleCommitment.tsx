...
interface ILifestyleCommitmentProps {
  onNext: (data: LifestyleFormType) => void;
  onBack: () => void;
  defaultValues?: LifestyleFormType;
}

const options = ["Yes", "No"];

const LifestyleSchema = yup.object({
  lifestyleCommitment: yup.string().required("Please select an option."),
});

type LifestyleFormType = yup.InferType<typeof LifestyleSchema>;

const LifestyleCommitment = ({ onNext, onBack, defaultValues }: ILifestyleCommitmentProps) => {
  const {
    handleSubmit,
    setValue,
    watch,
    formState: { errors },
  } = useForm<LifestyleFormType>({
    defaultValues: {
      lifestyleCommitment: defaultValues?.lifestyleCommitment || "",
    },
...
export default LifestyleCommitment;
