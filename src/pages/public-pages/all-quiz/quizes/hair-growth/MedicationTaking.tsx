...
const MedicationTaking = ({ onNext, onBack, defaultValues }: IMedicationTakingProps) => {
  ...
  return (
    ...
    >
      <div>
        <h2 className="text-center text-3xl font-poppins font-semibold text-foreground">Are you currently taking medications known to cause hair loss?</h2>

        <Radio.Group
          value={medicationTaking}
          onChange={handleSelect}
          className="mt-6"
          error={errors?.medicationTaking?.message}
        >
          <Group grow>
            {options.map((option) => (
              <Radio
                ...
                classNames={{
                  ...
                  label: `
                    ...
                    ${medicationTaking === option ? "border-primary bg-white text-black" : "border-grey bg-transparent text-black"}
                  `,
                }}
                label={
