...
const PriceUpdateHistory = ({ openModal, onModalClose, partnerId }: ModalProps) => {
  ...
  const fetchPriceHistory = () => {
    ...
    return medicineRepository.getCustomerPriceHistoryList(params);
  };
  ...
  const rows = medicineHistoryList?.map((item, index) => (
    <Table.Tr key={index}>
      <Table.Td className="!pl-0">{item.medication?.name}</Table.Td>
      ...
      <Table.Td className="!pl-0 text-end">${item.new_price}</Table.Td>
    </Table.Tr>
  ));
  return (
    <Modal.Root
            ...
            >
              <Table.Thead className="border-b border-grey-low">
                <Table.Tr>
                  ...
                  <Table.Th className="!pl-0 text-nowrap">Price Update Date</Table.Th>
                  <Table.Th className="!pl-0 text-nowrap">Previous Price</Table.Th>
                  <Table.Th className="!pl-0 text-nowrap text-end">Update Price</Table.Th>
                </Table.Tr>
              </Table.Thead>
              <Table.Tbody>
