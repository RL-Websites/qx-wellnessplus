...
import { ActionIcon, Button, Group, Image, Input, NumberInput, Radio, Text } from "@mantine/core";
...
interface StepOnePropTypes {
  patientDetails?: IPublicPartnerPrescriptionDetails;
  onNext: (data) => void;
  isSubmitting?: boolean;
}

const StepOne = ({ patientDetails, onNext, isSubmitting }: StepOnePropTypes) => {
  ...
  const partnerSlug = params.get("partner_slug");
  ...
  useEffect(() => {
    ...
    setPhone(tempPatientDetails?.patient?.cell_phone);
    setValue("phone", tempPatientDetails?.patient?.cell_phone || "", { shouldValidate: true });
