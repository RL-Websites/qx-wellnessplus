...
function MatrixCard(cardProps: IMatrixCard) {
  return (
    <div className={`p-5 rounded-xl ${cardProps.matCard}`}>
      ...
      <div className={`heading-xl pt-1 pb-5 text-foreground font-uber ${cardProps.countStyle}`}>{cardProps.cardCount}</div>
      <p className={`text-fs-sm text-foreground !font-normal ${cardProps.textW}`}>{cardProps.cardText}</p>
      {cardProps?.cardLink && (
        <Link
          to={`${cardProps.cardLink}`}
          className="text-fs-md text-foreground !leading-[22px] !font-bold pt-4 font-uber flex items-center gap-1"
        >
          {cardProps.linkText}
